...
TYPE_DIGITAL_SENSOR = "digital_sensor" # Digital
TYPE_TANK_LEVEL	= "tl" # nivel do tanque
...
class CayenneMQTTClient:
    ...
    * Receive and process data from Cayenne in the on_message callback.
    ...
    def begin(self, username, password, clientid, hostname='mqtt.mydevices.com', port=1883, logname=LOG_NAME, loglevel=logging.WARNING):
        ...
        self.log.info("Connecting to {}:{}".format(hostname, port))
    ...
    def loop(self):
        ...
        if not self.connected and self.reconnect:
            try:
                ...
                self.log.info("Reconnect failed, retrying")
                time.sleep(5)
    ...
    def tankWrite(self, channel, value):
        
        self.virtualWrite(channel, value, TYPE_TANK_LEVEL, UNIT_PERCENT)
    ...
    def umidWrite(self, channel, value):
        """Send a Celsius value to Cayenne.
    ...
    def client_on_disconnect(self):
        ...
        def on_disconnect(client, cayenne, rc):
            self.log.info("Disconnected with result code {}".format(rc))
